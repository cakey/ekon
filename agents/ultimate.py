"""
Ultimate - Combined best discoveries + optimized parameters

=== EXPERIMENT FINDINGS ===

Iteration 36: Combined discoveries + parameter tuning.

Key changes from max_profit:
1. Backtrack avoidance: don't return to previous node when scoring neighbors
2. Global weight: 0.8 (was 0.5) - weight global opportunities higher
3. Sell thresholds: 0.70-0.95 (was 0.60-0.90) - more patient selling
...
- +$1,325/r more profit (+13.7%)
- Faster (0.0161ms vs 0.0172ms)
"""
...
def agent(ws, state, *a, **k):
    ...
    # Backtrack avoidance: exclude previous position from candidates
    prev = state.get('prev')
    candidates = [n for n in neighbors if n != prev] if len(neighbors) > 1 else neighbors

    # Global weight 0.8 (was 0.5) - weight global opportunities higher
    scored = [(edge_score(n) + 0.8 * global_score(n), n) for n in candidates]
    ...
    # Cash-adaptive sell threshold (70%->95%) - more patient selling
    if coin < 500:
        sell_thresh = 0.70
    elif coin > 10000:
        sell_thresh = 0.95
    ...
    # Leftover: global arb buys at 85%
    if budget > 0:
        for res, info in my_shop.items():
            qty, price = info['quantity'], info['sell']
